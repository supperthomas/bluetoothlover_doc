...
由两种元素组成：Node(节点)、Property(属性)

/chosen: 为zephyr kernel 选择特定的设备（如日志串口）



驱动中注意下面这个宏，用来指定是用的那个devicetree
...
刚开始接触设备树肯定会有一些人比较反感，比较反对。这里我来将一个本质的东西，在zephyr里面，设备树你可以就理解为宏定义，像我们写bsp驱动或者代码的时候，经常要把波特率，引脚配置啥的搞成宏定义方便大家来看，而不是光秃秃的数字。设备树本质上也就是产生这些宏定义。刚开始可以这样来理解，不过后面理解深刻了之后，会有更多的解读。
...
就是(0x4600+0)+0xe0000000,   最后一个代表的是地址取址范围。
